...
$sql = "SELECT  peminjaman.id, users.f_name, users.l_name,buku.nama,buku.img FROM peminjaman INNER JOIN buku ON buku.id=peminjaman.book_id INNER JOIN users ON users.id WHERE users.id = $id_user ";
$stmt = $db->prepare($sql);
$stmt->execute();
                        ...
                        ?>
                            <div class="card align-items-center" style="width: 18rem; margin-right: 5px; background-color:#f48120;">
                                    ...
                                    <div class="card-body">
										...
										<p class="card-text" style="color:white;">-Belum Dikembalikan</p>
                                    <?php
                           echo '</div>';
                        }
                        ?>
