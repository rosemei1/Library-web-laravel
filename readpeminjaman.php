...
$sql = "SELECT  peminjaman.id, users.f_name, users.l_name,buku.nama,buku.img FROM peminjaman INNER JOIN buku ON buku.id=peminjaman.book_id INNER JOIN users ON users.id WHERE users.role = 0 ";
$stmt = $db->prepare($sql);
$stmt->execute();
...
<html lang="en">
	...
	echo "<h2 style='color:white;'>Daftar Peminjaman</h2>";
	echo '<table border="1" cellpadding="10" id="bawah" style="background-color: white; ">';
	echo "<tr style='background-color: #195681; '>
				<th style='color:white;border: 1px solid white;'>Nama</th>
				<th style='color:white;border: 1px solid white;'>Buku Yang Dipinjam</th>
                <th style='color:white;border: 1px solid white;'>Action</th>
		  	</tr>";
	//jika true
	foreach ($user as $value) { 
		?>
			<tr>
				<td style="border: 1px solid #195681;"><?php echo $value['f_name']." ".$value['l_name']; ?></td>
				<td style="border: 1px solid #195681;"><?php echo $value['nama']; ?> </td>
                <td style="border: 1px solid #195681;"><a onclick="return confirm('Apakah anda ingin menghapus data ini ?')" href="deletepeminjaman.php?id=<?= $value['id'] ?>" class="btn btn-danger">Hapus</a></td>
			</tr>
		<?php
	}
	echo "</table>";
